  ...
  return (
    <section className="weather p-6 bg-gray-100  flex flex-col items-center">
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
        <h2 className="text-2xl font-bold text-green-700 text-center">Current Weather</h2>
        <div className="flex space-x-2 mt-4">
          ...
          />
          <button onClick={handleSearch} className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">Search</button>
        </div>
        <button onClick={() => navigator.geolocation.getCurrentPosition(fetchWeatherByLocation, showError)} className="w-full mt-3 bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600">
          Use Current Location
        </button>
        {weather && (
          <div className="mt-6 flex flex-col items-center">
            <img src={`https://openweathermap.org/img/wn/${weather.weather[0].icon}@4x.png`} alt={weather.weather[0].description} className="w-20 h-20" />
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 w-full mt-6 flex flex-col justify-center items-center">
        <h2 className="text-2xl font-bold text-green-700 text-center">5-Day Forecast</h2>
        <div className="mt-4 flex overflow-x-auto space-x-4 p-2">
          {forecast.map((item, index) => (
            <div key={index} className="flex flex-col items-center bg-gray-50 p-4 rounded-lg shadow min-w-[130px]">
              <img src={`https://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`} alt={item.weather[0].description} className="h-12" />
